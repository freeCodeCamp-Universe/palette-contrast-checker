...
describe('results view — badges', () => {
  ...
  it('renders badges as a list (ul/li) split into AA and AAA levels', () => {
    ...
    expect(text[0]).toContain('Normal text AAA');
    expect(text[1]).toContain('Normal text AA');
    expect(text[2]).toContain('Large text AAA');
    expect(text[3]).toContain('Large text AA');
  ...
  it('marks AA pass / AAA fail separately and colors them green/red', () => {
    // normalText 'AA' => fails AAA, passes AA; nonText 'fail'
    ...
    expect(items[0].classList.contains('badge-fail')).toBe(true); // Normal AAA
    expect(items[0].textContent).toContain('Fail');
    expect(items[0].textContent).toContain('✗');
    expect(items[1].classList.contains('badge-pass')).toBe(true); // Normal AA
